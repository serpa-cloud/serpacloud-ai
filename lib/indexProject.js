    ...
    for (const file of files) {
      ...
      if (fileStat.isDirectory()) {
        ...
        if (!isInCache || cacheData[relativePath].hash !== fileHash || forceRefresh) {
          ...
          if (filesBatch.length === 4) {
            await sendBatch(filesBatch, cacheData, cacheFilePath, token);
            filesBatch.length = 0;
            await new Promise((resolve) => setTimeout(resolve, 15000)); // Desfase de 40 segundos
          }
        }
    ...
    if (fileStat.isDirectory()) {
      ...
      if (!isInCache || cacheData[includePath].hash !== fileHash || forceRefresh) {
        ...
        if (filesBatch.length === 3) {
          await sendBatch(filesBatch, cacheData, cacheFilePath, token);
          filesBatch.length = 0;
          await new Promise((resolve) => setTimeout(resolve, 15000)); // Desfase de 30 segundos
        }
      }
