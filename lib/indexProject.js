 ...
 */
async function getDirectoryStructure(dir, ignoreFile = '.scignore') {
  ...
  async function readDirectory(currentDir) {
    ...
    for (const file of files) {
      ...
      if (fileStat.isDirectory()) {
        ...
        const pathHash = generatePathHash(relativePath); // Generar hash del pathname
        const isInCache = !!cacheData[relativePath];

        if (!isInCache || forceRefresh) {
  ...
  if (filesBatch.length > 0) {
    await sendBatch(filesBatch); // Enviar cualquier archivo que quede fuera del batch de 20
  }
